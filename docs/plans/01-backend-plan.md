...
1. Criar DTOs como `record` do Java 17:
   ...
   - `CreateSessionRequest(String title)`
...
1. Criar `SessionService`:
   ...
   - `createSession(CreateSessionRequest)` → cria `Session`, persiste, retorna `SessionDTO`
...
3. Criar `ChatService`:
   - `sendMessage(ChatRequest)` valida sessão, persiste mensagem do usuário, invoca `AiResponseGenerator`, persiste resposta, retorna `ChatResponse` com ambas as mensagens
